export const LensDistortionShader = {
    ...
    uniforms: {
        ...
        mousePos: { value: new THREE.Vector3(0, 0, 0) } // Uniform para la posición del mouse
    ...
    fragmentShader: /* glsl */`
        ...
        uniform vec3 mousePos; // Uniform para la posición del mouse
        ...
        void main() {
            ...
            float distToMouse = length(gl_FragCoord.xyz - mousePos); // Distancia a la posición del mouse

            if(distToMouse < 1.5) { // Ajusta el valor según la sensibilidad que desees
                // Aplica una distorsión adicional si está cerca del mouse
                float distortionFactor = clamp(1.0 / (distToMouse * distToMouse), 0.0, 0.5);
                normal += normalize(mousePos - gl_FragCoord.xyz) * distortionFactor;
            }
